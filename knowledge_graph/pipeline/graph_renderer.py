...
from knowledge_graph.pipeline.types import AggregatedRelation
from knowledge_graph.runtime_assets import ensure_runtime_assets
from knowledge_graph.settings import GRAPH_ASSETS_DIR
...
def write_graph_bundle(graph: nx.DiGraph, relations: Sequence[AggregatedRelation], run_dir: Path) -> tuple[Path, Path, Path]:
    ...
    html_path.write_text(
        _render_viewer_html(
            vis_network_css=f'<link rel="stylesheet" href="/static/vendor/{VIS_NETWORK_CSS_FILE_NAME}">',
            graph_data_script=f'<script src="./{GRAPH_DATA_FILE_NAME}"></script>',
            options=options,
            vis_network_js=f'<script src="/static/vendor/{VIS_NETWORK_JS_FILE_NAME}"></script>',
        ),
        encoding="utf-8",
    )
