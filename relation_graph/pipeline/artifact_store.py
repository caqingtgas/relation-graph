...
from relation_graph.settings import MAX_SUCCESSFUL_RUNS, RUNS_DIR, RUN_TEMP_PREFIX, UPLOAD_TEMP_PREFIX
...
def save_selected_files(file_paths: Sequence[str], *, max_total_bytes: int | None = None) -> SavedUploadBatch:
    saved_sources: list[UploadedSource] = []
    total_bytes = 0
    for raw_path in file_paths:
        path = Path(str(raw_path or "").strip())
        if not path.exists() or not path.is_file():
            raise ValueError(f"文件不存在或不可读取：{path}")
        ...
        if max_total_bytes is not None and total_bytes > max_total_bytes:
            raise ValueError(f"上传文件总大小超过上限 {max_total_bytes // (1024 * 1024)} MB，请减少文件后重试。")
        saved_sources.append(UploadedSource(path=path, original_name=path.name))
    return SavedUploadBatch(temp_dir=None, sources=saved_sources, total_bytes=total_bytes)


def cleanup_saved_upload_batch(batch: SavedUploadBatch | None) -> None:
    if batch is None or batch.temp_dir is None:
        return
