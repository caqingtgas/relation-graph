...
class RelationGraphDesktopService:
    ...
    def bind_model_dir(self, params: dict[str, Any]) -> dict[str, object]:
        ...
        try:
            ...
            raise DesktopServiceError(str(exc)) from exc
        except Exception as exc:
            logger.exception("绑定已有模型目录失败")
            raise DesktopServiceError("绑定已有模型目录失败，请稍后重试。") from exc
    ...
    def download_models(self, params: dict[str, Any]) -> dict[str, object]:
        ...
        try:
            ...
            raise DesktopServiceError("下载模型并配置目录失败，请稍后重试。") from exc
    ...
    def ensure_started(self) -> dict[str, object]:
        try:
            ...
            raise DesktopServiceError("启动本地模型失败，请稍后重试。") from exc
    ...
    def launch_runtime_terminal(self) -> dict[str, object]:
        try:
            ...
            raise DesktopServiceError("打开本地引擎终端失败，请稍后重试。") from exc
    ...
    def set_preferred_model(self, params: dict[str, Any]) -> dict[str, object]:
        ...
        try:
            ...
            raise DesktopServiceError("切换本地模型失败，请稍后重试。") from exc
    ...
    def submit_job(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        if not files:
            raise DesktopServiceError("请先上传文件。")
        if len(files) > MAX_FILES_PER_JOB:
            raise DesktopServiceError(f"单次最多上传 {MAX_FILES_PER_JOB} 个文件，请减少后重试。")
        ...
        try:
            ...
            raise DesktopServiceError("读取已选文件失败，请稍后重试。") from exc

        if not saved_batch.sources:
            cleanup_saved_upload_batch(saved_batch)
            raise DesktopServiceError("暂时只支持 pdf、txt、md 文件。")
        ...
        try:
            ...
            raise DesktopServiceError("图谱任务创建失败，请稍后重试。") from exc
    ...
    def get_job_status(self, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise DesktopServiceError("任务不存在。") from exc
    ...
    def _require_string(params: dict[str, Any], key: str) -> str:
        ...
        if not value:
            raise DesktopServiceError(f"缺少参数：{key}")
        return value
    ...
    def _require_string_list(params: dict[str, Any], key: str) -> list[str]:
        ...
        if not isinstance(value, list):
            raise DesktopServiceError(f"参数格式错误：{key}")
