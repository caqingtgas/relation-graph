...
from knowledge_graph.pipeline.chunking import split_text
from knowledge_graph.pipeline.document_loader import _clean_pdf_text
from knowledge_graph.pipeline.artifact_store import cleanup_stale_runtime_files, write_pipeline_result
from knowledge_graph.pipeline.graph_renderer import write_graph_bundle
from knowledge_graph.pipeline.relation_aggregation import aggregate_relations, build_graph
from knowledge_graph.pipeline.relation_service import extract_relations_for_chunks
from knowledge_graph.pipeline.types import AggregatedRelation, ChunkExtractionSummary, PreparedChunk, RawRelationRecord, WarningDetail
from knowledge_graph import runtime_assets
from knowledge_graph.kg_models import canonical_text_key
...
def test_graph_html_uses_public_static_assets(tmp_path: Path):
    graph = build_graph(
        [
            AggregatedRelation(
                node_1="A",
    ...
    assert '/static/vendor/vis-network.min.css' in html
    assert '/static/vendor/vis-network.min.js' in html
    assert '/static/vendor/' not in standalone


def test_write_pipeline_result_persists_warning_details(tmp_path: Path, monkeypatch):
    monkeypatch.setattr("knowledge_graph.pipeline.artifact_store.RUNS_DIR", tmp_path)
    graph = build_graph([])
...
def test_cleanup_stale_runtime_files_uses_stable_prefixes(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("knowledge_graph.pipeline.artifact_store.RUNS_DIR", runs_dir)
    monkeypatch.setattr("knowledge_graph.pipeline.artifact_store.UPLOAD_TEMP_PREFIX", "kg-ui-")
    monkeypatch.setattr("knowledge_graph.pipeline.artifact_store.RUN_TEMP_PREFIX", ".tmp-kg-run-")
...
def test_ensure_runtime_assets_validates_repo_assets_only(tmp_path: Path, monkeypatch):
    ...
    static_vendor_dir = tmp_path / "vendor"
    graph_assets_dir.mkdir()
    static_vendor_dir.mkdir()

    for parent in (graph_assets_dir, static_vendor_dir):
        (parent / runtime_assets.VIS_NETWORK_JS_FILE_NAME).write_text("ok", encoding="utf-8")
        (parent / runtime_assets.VIS_NETWORK_CSS_FILE_NAME).write_text("ok", encoding="utf-8")

    monkeypatch.setattr(runtime_assets, "GRAPH_ASSETS_DIR", graph_assets_dir)
    monkeypatch.setattr(runtime_assets, "STATIC_VENDOR_DIR", static_vendor_dir)
...
def test_provider_status_endpoint_is_read_only(monkeypatch):
    ...
    monkeypatch.setattr(webapp.local_provider_manager, "get_public_status", fake_status)
...
def test_ensure_started_endpoint_routes_to_manager(monkeypatch):
    ...
    client = TestClient(webapp.app)
    response = client.post("/local-provider/ensure-started")
